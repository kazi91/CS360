 ...
 * performing operations such as user registration, login, role management, and now handling
 * feedback (used as reviews), private messaging, trusted reviewer management, and reviewer role requests.
 */
...
//			statement.execute("DROP TABLE IF EXISTS Questions;");
	...
	private void createTables() throws SQLException {
		...
		String messagesTable = "CREATE TABLE IF NOT EXISTS messages (" +
				...
				"FOREIGN KEY (receiver) REFERENCES cse360users(userName))";
	...
	public List<String[]> getFeedbackByReviewer(int reviewerId) throws SQLException {
		List<String[]> feedbackList = new ArrayList<>();
		String query = "SELECT f.id, f.questionId, f.feedbackText, f.timestamp FROM Feedback f WHERE f.reviewerId = ?";
		try (PreparedStatement pstmt = connection.prepareStatement(query)) {
	...
	public void updateFeedback(int feedbackId, int reviewerId, String newFeedbackText) throws SQLException {
		if (newFeedbackText == null || newFeedbackText.trim().isEmpty()) {
			throw new IllegalArgumentException("Feedback cannot be empty.");
		}
		// Check that the reviewerId matches the original feedback's reviewerId.
		String query = "UPDATE Feedback SET feedbackText = ? WHERE id = ? AND reviewerId = ?";
		try (PreparedStatement pstmt = connection.prepareStatement(query)) {
	...
	public void deleteFeedback(int feedbackId, int reviewerId) throws SQLException {
		// Check that the reviewerId matches before deleting.
		String query = "DELETE FROM Feedback WHERE id = ? AND reviewerId = ?";
		try (PreparedStatement pstmt = connection.prepareStatement(query)) {
			pstmt.setInt(1, feedbackId);
	...
	public List<String[]> getMessagesBetween(String user1, String user2) throws SQLException {
		List<String[]> messages = new ArrayList<>();
		String query = "SELECT sender, receiver, content, FORMATDATETIME(timestamp, 'yyyy-MM-dd HH:mm') as formatted_time " +
	...
	public void updateTrustedReviewerWeight(int studentId, int reviewerId, int weight) throws SQLException {
		String query = "UPDATE TrustedReviewers SET weight = ? WHERE studentId = ? AND reviewerId = ?";
		try (PreparedStatement pstmt = connection.prepareStatement(query)) {
	...
	public List<TrustedReviewer> getTrustedReviewers(int studentId) throws SQLException {
		List<TrustedReviewer> list = new ArrayList<>();
		String query = "SELECT tr.studentId, tr.reviewerId, tr.weight, u.userName FROM TrustedReviewers tr JOIN cse360users u ON tr.reviewerId = u.id WHERE studentId = ? ORDER BY weight DESC";
		try (PreparedStatement pstmt = connection.prepareStatement(query)) {
			pstmt.setInt(1, studentId);
			ResultSet rs = pstmt.executeQuery();
			while (rs.next()) {
				list.add(new TrustedReviewer(
						rs.getInt("studentId"),
	...
	// ----------------------- REVIEWER ROLE REQUESTS -----------------------

	public void createReviewerRequest(int studentId, String requestText) throws SQLException {
		if (requestText == null || requestText.trim().isEmpty()) {
			throw new IllegalArgumentException("Request text cannot be empty.");
		}
		String query = "INSERT INTO ReviewerRequests (studentId, requestText) VALUES (?, ?)";
		try (PreparedStatement pstmt = connection.prepareStatement(query)) {
			pstmt.setInt(1, studentId);
	...
	public List<ReviewerRequest> getReviewerRequests() throws SQLException {
		List<ReviewerRequest> requests = new ArrayList<>();
		String query = "SELECT r.id, r.studentId, u.userName as studentName, r.requestText, r.status, r.timestamp " +
				"FROM ReviewerRequests r JOIN cse360users u ON r.studentId = u.id WHERE r.status = 'pending'";
		try (Statement stmt = connection.createStatement();
			 ResultSet rs = stmt.executeQuery(query)) {
			while (rs.next()) {
